...
| **Provider** | A marketplace source such as `steam`, `skinport`, or `buff163`. | provider key, fee data, health metadata | [`/providers`](/api-reference/catalog#get-/providers-—-list-providers) |
...
[`/prices`](/api-reference/prices) and [`/bids`](/api-reference/bids) are served from a pre-built index, not from live per-request marketplace queries. This means:
...
<AccordionGroup>
  ...
  <Accordion title="Assuming prices and bids fall back to a live non-indexed path">
    [`/prices`](/api-reference/prices) and [`/bids`](/api-reference/bids) have no live fallback. A `503` response means the index is temporarily unavailable — not that the item doesn't exist or that you should query a different endpoint. Implement retry logic and surface the outage to your users if it persists.
  </Accordion>
</AccordionGroup>