...
- **Batch requests wherever possible.** Use `POST /v1/prices/batch` and `POST /v1/bids/batch` (Starter, Pro, and Quant) to fetch up to 100 items in a single request instead of making one request per item.
- **Use `POST /v1/prices` for bulk data only when your tier includes it.** When you need a full market snapshot, one call to `POST /v1/prices` costs a single request against your quota, regardless of how many items it returns. Schedule it no more than once every 30 seconds.
