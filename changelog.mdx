...
## 2026-09 — `stale` flag on prices and bids

Prices and bids rows now say whether they are still confirmed by the provider.

- Every row from `GET /v1/prices`, `POST /v1/prices`, `POST /v1/prices/batch`, `GET /v1/bids`, `POST /v1/bids`, and `POST /v1/bids/batch` includes `stale`. It is `true` when the provider's most recent scan did not include the listing or buy order, so the price shown is the last one observed and may already be gone.
- New `exclude_stale` option (query parameter on the list and stream endpoints, body field on the batch endpoints) leaves those rows out. It defaults to `false`, so existing integrations see the same rows as before.
- With `exclude_stale=true`, `pagination.total` on broad listings (no `item_id` or `market_hash_name`) and `X-Snapshot-Total` on the streams still count stale rows, so a page can contain fewer than `limit` items.
