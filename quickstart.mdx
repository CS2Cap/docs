...
<Steps>
  ...
  <Step title="Understand the response">
    ...
    The `/prices` response has three top-level fields:
    ...
    - `pagination` — paging state. Cursor endpoints such as `/sales` and `/prices/history` use `next_cursor` and report `total: -1`; do not treat that value as a record count.

    The following illustrative response shows a separate Doppler query, not the AK-47 request above.
  ...
  <Step title="Common next requests">
    ...
    curl -sS -H "Authorization: Bearer $CS2C_API_KEY" \
      "https://api.cs2c.app/v1/items?market_hash_name=AK-47%20%7C%20Redline%20(Field-Tested)"

    # Highest buy orders across buy-order-enabled providers (Starter/Pro/Quant)
    curl -sS -H "Authorization: Bearer $CS2C_API_KEY" \
      "https://api.cs2c.app/v1/bids?market_hash_name=AK-47%20%7C%20Redline%20(Field-Tested)&providers=steam"

    # Completed sale transactions from a specific provider (Pro/Quant)
    curl -sS -H "Authorization: Bearer $CS2C_API_KEY" \
      "https://api.cs2c.app/v1/sales?market_hash_name=AK-47%20%7C%20Redline%20(Field-Tested)&providers=csfloat&limit=10"

    # Historical price snapshots for trend analysis (Pro/Quant)
    curl -sS -H "Authorization: Bearer $CS2C_API_KEY" \
      "https://api.cs2c.app/v1/prices/history?market_hash_name=AK-47%20%7C%20Redline%20(Field-Tested)&provider=steam&limit=50"
    ```
  ...
  <Step title="Handle common failures">
    ...
    |--------|------|---------------|
    | `401` | `AUTH_INVALID_API_KEY` | The key is not recognized. Missing headers, malformed credentials, and revoked keys have distinct codes; see [Authentication](/authentication). |
    ...
    | `503` | `PRICES_INDEX_UNAVAILABLE` or `BIDS_INDEX_UNAVAILABLE` | A data index is temporarily unavailable due to a refresh cycle or upstream issue. Retry with bounded exponential backoff; recovery time varies. |
    | `422` | `VALIDATION_ERROR` | One or more query parameters are missing, have the wrong type, or contain an invalid value. Check the response body for a `detail` field that identifies the offending parameter. |

    <Warning>
      Do not retry `401` or `422` errors without first fixing the underlying issue — they will not self-resolve. For these read requests, retry transient `503` errors with bounded backoff and respect `Retry-After` on `429`. Monthly quota exhaustion requires waiting for the reset or upgrading your plan.
    </Warning>
...
<CardGroup cols={2}>
  <Card title="Authentication" icon="key" href="/authentication">
    Learn how API keys work and how to rotate them.
  </Card>
