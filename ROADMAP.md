...
**Current state (last refreshed 2026-06-06):**

- Frontend: faceted search, item detail (history, market distribution, similar items, variants, cases, asks, images), marketcap (treemap, sparkline, movers), auth (Steam/Google/Discord), inventory tracker, alerts, watchlist, API key/billing/usage UI, ~40 marketplace landing pages, and a data-driven browse navigation system — cached `/api/browse-nav` + hover-rail mega-menu with Containers/Gear/Stickers-&-More hubs and index/detail pages for crates, collections, stickers, slabs, charms, graffiti, music kits, patches, collectibles (rarity/text facet filtering, `BrowseUnavailable` graceful degradation). An "Active sessions" device-management UI in account settings is in PR (cs2cap #41). Deal scanner still absent on FE.
- Backend: 40+ provider adapters, prices/sales/buy_orders services, liquidity scoring, cross-provider price-outlier rejection, portfolio + transactions + CSV import, account/api-key/webhook services, OAuth (with `/v1/web/auth/{provider}/link` account linking) over an `oauth_accounts` identity table + `account_merge_service`, Steam inventory parser, skinscanner/deals_service, 69 alembic migrations (latest `0069`). Durable web sessions: a `user_sessions` table (migration `0068`) mirroring the Redis session keyed by `sha256(token)`, with `/v1/account/sessions` list/revoke endpoints, throttled `last_seen`, and a 24h-grace cleanup worker. Web search: snapshot scores on `/v1/web/search`, query understanding with alias dictionary + relevance sort, isolated rate-limit bucket. Alerts: `user_alerts`→`user_alert_events`→`user_alert_deliveries` split with window-based dedupe, plus multi-channel delivery (custom/Discord/Telegram/Google Sheets). Sales ingest is a background service feeding a cache-only `/sales` endpoint.
...
1. **`compositions.ts` / `view-models.ts` typing tightness** — As new entities land (taxonomy, drop_source, media_asset), view-model adapters will balloon; promote per-entity adapters into a `view-models/` folder.
2. **Image pipeline for non-cdn assets** — `next.config.ts` allows only `cdn.cs2c.app`; planning provider screenshots, sticker overlays, or wear previews requires expanding the allowlist (or proxying through cdn.cs2c.app).
...
- **PostHog event taxonomy** *(was Tier 6 #6)* — All analytics event names centralized into `ANALYTICS_EVENTS` (`src/lib/analytics/events.ts`); the 20 inline `posthog.capture("…")` string literals across 10 files (login, watchlist, inventory, alerts, webhooks, api-keys, billing, inventory-value, high-intent nav) now reference the const. Event string *values* are unchanged, so historical PostHog data is preserved. An eslint `no-restricted-syntax` rule now forbids raw string literals in `posthog.capture()` so the centralization can't regress. cs2cap commits `be873eb` + `1c38b81`.
- **Marketplace SEO freshness blocks** *(was Tier 6 #2 `(seo)` landing-page generation)* — The `(markets)` SEO landing pages now surface live, `captured_at`-driven provider data in the hero. `SeoLandingPage` already fetched `/v1/web/providers` (`getProviders(300)`) and rendered a 4-tile stat grid (offers / items / coverage / value); this adds the two fields the catalog item named — a **fee** tile (primary fee auto-selected from `sell_fee` / `insta_sell_fee` / `trading_spread_fee`, with a derived "you keep X%" payout sub-line) and a **market-type** tile (appended as a pair to keep the 2-col grid even, 4→6 tiles), plus a **"synced Xm ago"** freshness caption driven by `health.last_checked_at`. All logic reuses existing helpers (`pickPrimaryFee` / `formatFee` / `FEE_LABELS` / `normalizeMarketType` / `statusDotClass` from `marketplaces-utils.ts`, `RelativeTime` for the live ISR-safe timestamp) — single-file change to `SeoLandingPage.tsx`, no API or backend changes. Pages stay fresh automatically via the existing 300s revalidate.
- **Edge-cache policy parity** *(was Tier 6 #2)* — Public API cache rules now live in one shared frontend module (`src/lib/api/cache-policy.ts`) instead of a hand-written proxy-only matcher. The `/api/cs2c` proxy imports `matchApiCachePolicy`, and `serverApi` consumes the same source for the matching `/v1/web/market/overview` default without changing existing cache durations, auth behavior, or Blob snapshot fallback behavior.
- **TanStack Query key registry cleanup** *(was Tier 6 #1)* — `src/lib/api/hooks.ts` now has root query keys for parameterized watchlist, alerts, and alert-events query families, and mutation-driven invalidations use `queryKeys` instead of raw literals while preserving broad invalidation semantics.
- **Render-time currency boundary audit** *(was Tier 6 #2)* — FX fetching remains isolated to the client-only `CurrencyContext.tsx` path. Server-rendered price strings use stable `formatPriceMinor` formatting for already-selected USD or billing currency amounts and do not perform user-preference conversion, avoiding cache fragmentation by currency.
- **Shared loading/error primitives + item-page deferred wrapper** *(was Tier 6 #4 `Loading/error-state primitives`)* — Repeated item/search loading skeletons now use `SkeletonLine` from `src/components/ui/skeleton.tsx` instead of local duplicate helpers, the item detail page routes async sections through a local `<DeferredSection>` wrapper while preserving existing fallbacks, catalog/inventory/item-route failure panels share `ErrorState` from `src/components/ui/error-state.tsx`, and deferred item sections render compact inline failures instead of escalating recoverable section fetch errors to the full route boundary.
- **Durable session hardening** *(was Tier 2 #1 `session` table with revocation + device fingerprint)* — New `user_sessions` table (migration `0068`) that mirrors the opaque Redis web session as a durable, auditable row. Security model: `token_hash = sha256(raw_cookie)` is the **shared key** for both the Redis entry and the table row, so the raw token lives only in the `cs2c_web_session` cookie (a DB leak can't be replayed) and revoking another device just deletes its Redis key. Each row stores user-agent + IP + created/last_seen/expires/revoked timestamps. Adds `/v1/account/sessions` endpoints (list active, revoke one, revoke all-other-devices), a throttled (~10 min) best-effort `last_seen` touch on the auth path that only writes when a row actually changes, and a periodic cleanup worker purging expired/revoked rows past a 24h grace. Redis stays the per-request auth hot path; the table is the durable mirror. cs2c-api **PR #28 (merged + deployed)**; the frontend "Active sessions" management UI in account settings is cs2cap **PR #41 (open)**. Deferred follow-ups: new-device / anomaly alerts, and revocation of the stateless `cs2c_access_token` JWT (unrevocable until expiry without a separate jti-blocklist). Cutover note: the Redis key scheme change (raw → hashed) forced a one-time re-login on deploy.
...
- `cs2c-api/alembic/versions/` (70 migrations; new ones for each Tier 1–5 entity)
...
3. Decide migration safety (the platform has 69 alembic migrations and live data — additive-then-backfill-then-cutover is the safe pattern).
