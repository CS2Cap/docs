...
2. **`(seo)` landing-page generation** — `landing-pages.ts` is the right shape; introduce per-marketplace `captured_at`-driven content blocks (fee, payout, last sync) so SEO pages stay fresh automatically.
3. **Image pipeline for non-cdn assets** — `next.config.ts` allows only `cdn.cs2c.app`; planning provider screenshots, sticker overlays, or wear previews requires expanding the allowlist (or proxying through cdn.cs2c.app).
4. **Shared error-state primitive** — As deferred item-page sections multiply, a shared error-state primitive should become the standard failure surface to keep PPR friendly. *(Partial: shared skeleton line primitive + item-page `<DeferredSection>` wrapper shipped; see Recently shipped.)*
...
- **Shared skeleton line primitive + item-page deferred wrapper** *(partial — was Tier 6 #4 `Loading/error-state primitives`)* — Repeated item/search loading skeletons now use `SkeletonLine` from `src/components/ui/skeleton.tsx` instead of local duplicate helpers, and the item detail page routes async sections through a local `<DeferredSection>` wrapper while preserving existing fallbacks. Shared error-state standardization remains a follow-up.
