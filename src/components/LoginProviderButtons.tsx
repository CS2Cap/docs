...
const oauthProviders = [
  {
    id: "steam",
    name: "Steam",
    icon: <Image src="/assets/steam.svg" alt="Steam" width={20} height={20} className="invert" />,
  },
  {
    id: "google",
    name: "Google",
...
export function LoginProviderButtons({ apiBaseUrl }: { apiBaseUrl: string }) {
  return (
    <div className="space-y-3">
      {oauthProviders.map((provider) => (
        ...
        >
          {provider.icon}
          <span>SIGN IN WITH {provider.name.toUpperCase()}</span>
        </a>
      ))}
    </div>
  );
