...
| `market_hash_name` | string | Canonical Steam market hash name, for example \`★ Gut Knife | Fade (Factory New)`. Use this when you do not have an `item\_id\` yet. |
...
| `last_updated` | datetime | ISO 8601 UTC timestamp for when CS2Cap last ingested or processed the record. This differs from `timestamp`, which reflects the provider-side observation time. |
...
These fields are returned by `/items`. All are nullable — a `null` value means the field is either not applicable to that item or not yet known.
