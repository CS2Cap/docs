...
<AccordionGroup>
  ...
  <Accordion title="Rate limit errors">
    ...
    | `RATE_LIMIT_FEED_CONNECTIONS_EXCEEDED` | 429 | You already have the maximum number of concurrent [`/feed/prices`](/api-reference/feed#get-/feed/prices-—-stream-price-changes) or [`/feed/bids`](/api-reference/feed#get-/feed/bids-—-stream-bid-changes) connections open for this API key (2 by default). Close an existing connection or wait for the `Retry-After` seconds. |
