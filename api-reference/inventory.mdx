...
with cs2cap.ApiClient(configuration) as client:
    ...
    response = api.fetch_steam_inventory()
...
const response = await api.fetchSteamInventory();
...
curl -sS \
  -H "Authorization: Bearer $CS2C_API_KEY" \
  "$CS2C_API_BASE/inventory/steam/lookup?steam_id=anomaly"
```
...
with cs2cap.ApiClient(configuration) as client:
    api = cs2cap.InventoryApi(client)
    response = api.fetch_steam_inventory_by_steam_id(
        steam_id="anomaly"
    )
...
const api = new InventoryApi(new Configuration({ accessToken }));
const response = await api.fetchSteamInventoryBySteamId({ steamId: "76561198000000000" });
