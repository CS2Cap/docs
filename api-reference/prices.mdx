...
curl -sS \
  ...
  "https://api.cs2c.app/v1/prices?item_id=4994&providers=buff163&providers=csfloat&currency=USD"
```
...
<ParamField query="exclude_stale" type="boolean" default="false">
  Leave out listings that the provider's most recent scan did not include. When listing without `item_id` or `market_hash_name`, `pagination.total` still counts them, so a page can contain fewer than `limit` items.
</ParamField>
...
<ResponseField name="stale" type="boolean">
  `true` when the provider's most recent scan did not include this listing. The price shown is the last one observed and may no longer be available. Pass `exclude_stale=true` to leave these rows out.
</ResponseField>
...
<ParamField query="exclude_stale" type="boolean" default="false">
  Leave out listings that the provider's most recent scan did not include. `X-Snapshot-Total` still counts them.
</ParamField>
...
curl -sS -X POST \
  -H "Authorization: Bearer $CS2C_API_KEY" \
  "https://api.cs2c.app/v1/prices?providers=buff163"
```
...
```ndjson
{"provider":"avanmarket","item_id":2,"market_hash_name":"'Blueberries' Buckshot | NSWC SEAL","phase":null,"lowest_ask":3460,"quantity":6,"link":"https://cs2c.app/r/avanmarket/2","url":"https://avan.market/en/market/cs/blueberries-buckshot-nswc-seal","timestamp":"2026-03-22T01:41:35.353783Z","last_updated":"2026-03-22T01:41:36.221322Z","stale":false}
{"provider":"buff163","item_id":2,"market_hash_name":"'Blueberries' Buckshot | NSWC SEAL","phase":null,"lowest_ask":3187,"quantity":84,"link":"https://cs2c.app/r/buff163/2","url":"https://buff.163.com/goods/835687?from=market","timestamp":"2026-03-22T00:08:50.544878Z","last_updated":"2026-03-22T00:54:22.230624Z","stale":false}
{"provider":"c5","item_id":2,"market_hash_name":"'Blueberries' Buckshot | NSWC SEAL","phase":null,"lowest_ask":3156,"quantity":57,"link":"https://cs2c.app/r/c5/2","url":"https://www.c5game.com/csgo/808836530722177024/item/sell","timestamp":"2026-03-22T01:27:20.020270Z","last_updated":"2026-03-22T01:42:46.706601Z","stale":false}
```
...
<ParamField body="exclude_stale" type="boolean" default="false">
  Leave out quotes for listings that the provider's most recent scan did not include. An item whose only quotes are left out is listed in `items_not_found`.
</ParamField>
...
curl -sS -X POST \
  ...
  "https://api.cs2c.app/v1/prices/batch"
```
...
curl -sS \
  -H "Authorization: Bearer $CS2C_API_KEY" \
  "https://api.cs2c.app/v1/prices/candles?item_id=4994&interval=1d&lookback=30d&currency=USD"
```
...
curl -sS \
  -H "Authorization: Bearer $CS2C_API_KEY" \
  "https://api.cs2c.app/v1/prices/history?item_id=4994&provider=buff163&currency=USD&limit=50"
```
