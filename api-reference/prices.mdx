...
Returns the current lowest ask across marketplaces. You can filter by item, phase, provider, and currency. Valid filters with no current matches return `200` with an empty `items` array; `503` indicates indexed data is unavailable or inconsistent.
...
<ParamField query="lookback" type="string">
  Duration shorthand such as `7d` or `30`. When sent with `start` or `end`, `lookback` takes precedence and selects a completed candle-bucket window. For `interval=1h`, the window ends at the hour boundary after a 15-minute freshness buffer.
</ParamField>
