...
const api = new PricesApi(new Configuration({ accessToken }));
const response = await api.priceCandles({
  itemId: 4994,
  interval: "1d",
  lookback: "30d",
  fill: true,
...
with cs2cap.ApiClient(configuration) as client:
    api = cs2cap.PricesApi(client)
    response = api.price_history(
        item_id=4994,
        provider=cs2cap.AllProviders.BUFF163,
        currency="USD",
        limit=50,
    )
...
const api = new PricesApi(new Configuration({ accessToken }));
const response = await api.priceHistory({
  itemId: 4994,
  provider: "buff163",
  currency: "USD",
  limit: 50,
