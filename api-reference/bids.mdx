...
description: "Query the highest active buy orders across CS2 marketplaces, stream the full live bids catalog as NDJSON, batch-lookup up to 100 items at once, or page through raw buy-order history."
---
...
curl -sS \
  ...
  "https://api.cs2c.app/v1/bids?item_id=4994&providers=buff163&providers=csfloat&currency=USD"
```
...
<ParamField query="exclude_stale" type="boolean" default="false">
  Leave out buy orders that the provider's most recent scan did not include. When listing without `item_id` or `market_hash_name`, `pagination.total` still counts them, so a page can contain fewer than `limit` items.
</ParamField>
...
<ResponseField name="stale" type="boolean">
  `true` when the provider's most recent scan did not include this buy order. The bid shown is the last one observed and may no longer be active. Pass `exclude_stale=true` to leave these rows out.
</ResponseField>
...
<ParamField query="exclude_stale" type="boolean" default="false">
  Leave out buy orders that the provider's most recent scan did not include. `X-Snapshot-Total` still counts them.
</ParamField>
...
curl -sS -X POST \
  -H "Authorization: Bearer $CS2C_API_KEY" \
  "https://api.cs2c.app/v1/bids?providers=buff163"
```
...
```ndjson
{"provider":"buff163","item_id":12632,"market_hash_name":"AK-47 | Redline (Field-Tested)","phase":null,"highest_bid":3001,"num_bids":232,"timestamp":"2026-03-21T05:22:39.570895Z","last_updated":"2026-03-21T05:22:43.400024Z","stale":false}
{"provider":"csfloat","item_id":12632,"market_hash_name":"AK-47 | Redline (Field-Tested)","phase":null,"highest_bid":2946,"num_bids":2,"timestamp":"2026-03-21T04:59:16.565601Z","last_updated":"2026-03-21T05:03:40.837477Z","stale":false}
{"provider":"steam","item_id":2,"market_hash_name":"'Blueberries' Buckshot | NSWC SEAL","phase":null,"highest_bid":289,"num_bids":4820,"timestamp":"2026-03-22T01:41:00.000000Z","last_updated":"2026-03-22T01:41:36.221322Z","stale":false}
```
...
<ParamField body="exclude_stale" type="boolean" default="false">
  Leave out quotes for buy orders that the provider's most recent scan did not include. An item whose only quotes are left out is listed in `items_not_found`.
</ParamField>
...
curl -sS -X POST \
  ...
  "https://api.cs2c.app/v1/bids/batch"
```
  ...
  <Expandable title="item properties">
    ...
    <ResponseField name="quotes" type="object[]">
      ...
      <Expandable title="quote properties">
        ...
        <ResponseField name="stale" type="boolean">
          `true` when the provider's most recent scan did not include this buy order. The bid shown is the last one observed and may no longer be active. Pass `exclude_stale=true` to leave these quotes out.
        </ResponseField>
      </Expandable>
    </ResponseField>
  </Expandable>
...
Returns historical buy-order records for one item, optionally filtered to a single provider. Results are ordered newest-first and use cursor pagination. Requests reaching back more than 35 days return hourly buckets; more than 365 days, daily buckets. History is available from September 2026 onward.
...
curl -sS \
  -H "Authorization: Bearer $CS2C_API_KEY" \
  "https://api.cs2c.app/v1/bids/history?item_id=4994&provider=csfloat&currency=USD&limit=50"
```
...
```typescript TypeScript
import { BidsApi, Configuration } from "cs2cap";

const accessToken = process.env.CS2C_API_KEY;

const api = new BidsApi(new Configuration({ accessToken }));
const response = await api.getBidHistory({
  itemId: 4994,
  provider: "csfloat",
  currency: "USD",
  limit: 50,
...
<ResponseField name="provider" type="string">
  Provider key for this record, for example `csfloat`.
</ResponseField>
