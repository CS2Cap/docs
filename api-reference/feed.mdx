...
curl -N -sS \
  ...
  "https://api.cs2c.app/v1/feed/prices?providers=steam&providers=csfloat&item_ids=17464"
```
...
with cs2cap.ApiClient(configuration) as client:
    ...
    for event in feed.stream_prices(providers=["steam", "csfloat"], item_ids=[17464]):
        for item in event.items:
            print(event.provider, item.item_id, item.change, item.price)
```

```typescript TypeScript
import { Configuration, FeedStream } from "cs2cap";
...
for await (const event of feed.streamPrices({
...
<Note>
  `FeedStream` opens one connection per call and yields events as they arrive.
  Leaving the loop closes the connection. Heartbeat comments are consumed for you, so
  every value you receive is a real event: `ready` first, then `prices.changed`.
</Note>
...
curl -N -sS \
  -H "Authorization: Bearer $CS2C_API_KEY" \
  "https://api.cs2c.app/v1/feed/bids?providers=steam&providers=buff163&item_ids=17464"
```
