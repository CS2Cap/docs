...
curl -sS -X POST \
  ...
  "https://api.cs2c.app/v1/portfolio"
```
...
<ParamField query="include" type="string">
  Set to `value` to add `total_value` and `change_24h_pct` to every portfolio in the response. Omitted by default, since valuing every portfolio costs an extra lookup per portfolio.
</ParamField>

<ResponseField name="data[].currency" type="string" default="USD">
  Portfolio base currency (ISO 4217). Values are returned in this currency.
</ResponseField>

<ResponseField name="data[].price_provider" type="string | null">
  Pinned pricing provider key, or `null` when the portfolio is valued at the best ask across all providers.
</ResponseField>

<ResponseField name="data[].total_value" type="integer | null">
  Current total value in minor units of the base currency. Present only with `include=value`.
</ResponseField>

<ResponseField name="data[].change_24h_pct" type="number | null">
  Percent change against the previous daily close. Present only with `include=value`, and `null` until a previous close exists.
</ResponseField>
...
curl -sS -X DELETE \
  -H "Authorization: Bearer $CS2C_API_KEY" \
  "https://api.cs2c.app/v1/portfolio/YOUR_PORTFOLIO_ID"
```
...
curl -sS -X POST \
  ...
  "https://api.cs2c.app/v1/portfolio/value"
```
...
curl -sS \
  -H "Authorization: Bearer $CS2C_API_KEY" \
  "https://api.cs2c.app/v1/portfolio/YOUR_PORTFOLIO_ID/value?currency=USD&providers=buff163"
```
...
curl -sS -X POST \
  ...
  -H "Content-Type: application/json" \
  -d '{}' \
  "https://api.cs2c.app/v1/portfolio/YOUR_PORTFOLIO_ID/import"
```
...
```json Request body (optional asset filter and cost basis)
{
...
<ParamField body="prices" type="object">
  Optional per-unit purchase price in minor units, keyed by Steam asset ID. Every imported asset with a price also gets a matching `buy` transaction — dated today, marketplace `steam`, in the portfolio's base currency — so the import carries cost basis. Assets you leave out are imported without one.
</ParamField>
...
curl -sS -X POST \
  ...
  "https://api.cs2c.app/v1/portfolio/YOUR_PORTFOLIO_ID/import/csv"
```
...
curl -sS \
  -H "Authorization: Bearer $CS2C_API_KEY" \
  -o portfolio.csv \
  "https://api.cs2c.app/v1/portfolio/YOUR_PORTFOLIO_ID/export?type=transactions"
```
...
curl -sS \
  -H "Authorization: Bearer $CS2C_API_KEY" \
  "https://api.cs2c.app/v1/portfolio/YOUR_PORTFOLIO_ID/items"
```
...
curl -sS -X DELETE \
  -H "Authorization: Bearer $CS2C_API_KEY" \
  "https://api.cs2c.app/v1/portfolio/YOUR_PORTFOLIO_ID/items/YOUR_ENTRY_ID"
```
...
curl -sS \
  -H "Authorization: Bearer $CS2C_API_KEY" \
  "https://api.cs2c.app/v1/portfolio/YOUR_PORTFOLIO_ID/transactions"
```
...
curl -sS -X PATCH \
  ...
  "https://api.cs2c.app/v1/portfolio/YOUR_PORTFOLIO_ID/transactions/YOUR_TX_ID"
```
...
curl -sS \
  -H "Authorization: Bearer $CS2C_API_KEY" \
  "https://api.cs2c.app/v1/portfolio/YOUR_PORTFOLIO_ID/history?lookback=30d&currency=USD"
```
