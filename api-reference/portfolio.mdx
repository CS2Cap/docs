...
with cs2cap.ApiClient(configuration) as client:
    api = cs2cap.PortfolioApi(client)
    response = api.portfolio_valuation(
...
const api = new PortfolioApi(new Configuration({ accessToken }));
const response = await api.portfolioValuation({
  portfolioRequest: {
    items: [
...
with cs2cap.ApiClient(configuration) as client:
    api = cs2cap.PortfolioApi(client)
    response = api.value_saved_portfolio(
        portfolio_id,
        currency="USD",
    )
...
const api = new PortfolioApi(new Configuration({ accessToken }));
const response = await api.valueSavedPortfolio({ portfolioId, currency: "USD" });
...
with cs2cap.ApiClient(configuration) as client:
    api = cs2cap.PortfolioApi(client)
    response = api.historical_saved_portfolio_valuation(
        portfolio_id,
        lookback="30d",
        currency="USD",
    )
...
const api = new PortfolioApi(new Configuration({ accessToken }));
const response = await api.historicalSavedPortfolioValuation({ portfolioId, lookback: "30d", currency: "USD" });
