...
The Account API lets you inspect your current API key's metadata — including tier, rate limits, and quota details — and reissue the key when you need to rotate your credentials. Reissuing a root key revokes all child keys under it in the same operation, making it a single step for a full credential rotation.
...
```bash Request
curl https://api.cs2c.app/v1/account/key \
...
Replaces your active root key and revokes the entire child-key tree under it in the same security event. Use this whenever you need to rotate credentials or suspect a key has been compromised.

**Tiers:** Free, Starter, Pro, Quant · **Auth:** Session JWT or root API key · **Rate limit:** Standard per-tier RPM

```bash Request
curl -X POST https://api.cs2c.app/v1/account/key/reissue \
  -H "Authorization: Bearer YOUR_API_KEY"
```
