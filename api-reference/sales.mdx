...
<ParamField query="item_id" type="integer">
  Restrict to one catalog item by ID. Takes precedence over `market_hash_name` and `phase` when provided. Omit all item filters to return the global recent-sales feed.
</ParamField>

<ParamField query="market_hash_name" type="string">
  Restrict to one item by exact name as it appears in inventory. Ignored when `item_id` is provided.
</ParamField>
...
<ParamField query="limit" type="integer">
  Page size. Defaults to and is capped at the tier maximum: **100** for `pro`, **1000** for `quant`.
</ParamField>

<ParamField query="cursor" type="string">
  Opaque cursor from the previous response's `pagination.next_cursor`. Returns the next page of results. Omit for the first page.
</ParamField>
