...
print(response.to_dict() if hasattr(response, "to_dict") else response)
```
...
const response = await api.listRecentSales({ itemId: 4994, providers: ["csfloat"], currency: "USD", limit: 20 });
