...
The Sales API returns the most recent completed transactions for a CS2 item across the supported marketplaces. Results include per-item detail that goes beyond price alone: float values, paint seeds, sticker wear, charm patterns, and inspect links when the provider exposes them. Sales data is refreshed by background ingestion and served from cache. Some catalog categories may be intentionally excluded from sales ingestion.
...
<Note>
  Background ingestion targets a 24 hour refresh interval per item/provider pair.
</Note>
