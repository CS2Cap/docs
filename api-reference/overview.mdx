...
[`POST /prices`](/api-reference/prices#post-/prices-—-stream-full-prices-snapshot) and [`POST /bids`](/api-reference/bids#post-/bids-—-stream-full-bids-snapshot) (the full-snapshot streaming endpoints) each have an additional **1 request per 5 minutes** cooldown per API key, regardless of tier.
