...
curl -sS \
  ...
  "https://api.cs2c.app/v1/market/arbitrage?min_spread_pct=5&providers_buy=csmoney_m&providers_sell=buff163&limit=25"
```
...
curl -sS \
  -H "Authorization: Bearer $CS2C_API_KEY" \
  "https://api.cs2c.app/v1/market/indexes?group_by=weapon_type"
```
...
curl -sS \
  -H "Authorization: Bearer $CS2C_API_KEY" \
  "https://api.cs2c.app/v1/market/indicators?item_id=4994&interval=1d&currency=USD"
```
...
curl -sS \
  -H "Authorization: Bearer $CS2C_API_KEY" \
  "https://api.cs2c.app/v1/market/items"
```
...
<ResponseField name="data.items[].summary.best_bid_usd" type="string">
  Decimal string highest eligible bid in USD across supported buy-order providers. Same-provider bids greater than or equal to the current ask are excluded; providers without a current ask remain eligible. The value can still exceed `best_ask_usd` when the winning bid and ask come from different providers.
</ResponseField>
...
<ResponseField name="data.items[].summary.price_change_confirmed_24h" type="boolean">
  Whether the 24h change is corroborated in direction and magnitude across multiple marketplaces. When `false`, treat the 24h change as unconfirmed.
</ResponseField>
...
<ResponseField name="data.items[].summary.listing_score" type="integer | null">
  Fresh competitive listing-depth component score (0–15). `null` when liquidity data is unavailable.
</ResponseField>

<ResponseField name="data.items[].summary.gap_score" type="integer | null">
  Fee-adjusted same-provider execution-quality component score (0–25), including bid depth. `null` when liquidity data is unavailable.
</ResponseField>

<ResponseField name="data.items[].summary.volume_score" type="integer | null">
  Smoothed confirmed 24h/7d/30d sales-velocity component score (0–45). `null` when liquidity data is unavailable.
</ResponseField>

<ResponseField name="data.items[].summary.stability_score" type="integer | null">
  Provider-normalized price-stability component score (0–5), combining observation density and within-provider variation over the last 24 hours. `null` when insufficient history.
</ResponseField>

<ResponseField name="data.items[].summary.external_score" type="integer | null">
  Provider, execution-book, and confirmed-sales data coverage score (0–10). `null` when liquidity data is unavailable.
</ResponseField>
...
curl -sS \
  -H "Authorization: Bearer $CS2C_API_KEY" \
  "https://api.cs2c.app/v1/market/items/4994"
```
...
<Note>
  `ask_usd`, `bid_usd`, `spread_usd`, and `total_value_24h_usd` are decimal strings in USD — not minor units. `summary.best_bid_usd` excludes same-provider bids where `bid_anomaly` is `true`; bid-only providers remain eligible and do not appear in `data.providers[]`. A summary bid can still exceed the summary ask when those values come from different providers.
</Note>
...
curl -sS \
  -H "Authorization: Bearer $CS2C_API_KEY" \
  "https://api.cs2c.app/v1/market/history/chart?item_id=16194&providers=buff163&providers=steam&currency=USD"
```
...
<ParamField query="lookback" type="string">
  Duration shorthand such as `7d` or `30`. Overrides `start` when provided.
</ParamField>
