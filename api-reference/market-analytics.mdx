...
<Note>
  `best_ask_usd` and all `price_*` values are decimal strings in USD — not minor units. Rows are sorted by rank, then by item ID. If the cached snapshot is stale, the API attempts to rebuild it and falls back to the last good snapshot if the refresh fails.
</Note>
