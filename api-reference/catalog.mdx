...
url.searchParams.set('q', 'AK-47');
url.searchParams.set('item_type', 'Rifle');
...
const response = await fetch(`${base}/items/metadata`, {
  headers: { Authorization: `Bearer ${key}` },
});
...
response.raise_for_status()
print(response.json()["AK-47 | Redline"])
```
...
console.log(images['AK-47 | Redline']);
```
