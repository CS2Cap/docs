...
curl -sS \
  ...
  "https://api.cs2c.app/v1/items?q=AK-47&rarity_name=Covert&limit=10"
```
...
<ParamField query="item_subtype" type="string">
  Filter by item subtype (e.g. `Knives`). Values depend on `item_type`; the full list comes
  from `GET /v1/items/metadata`. Stickers use `Player Autograph`, `Team Logo`, `Tournament`
  and `Other`; graffiti and patches use `Team Logo`, `Tournament` and `Other`; every sticker
  slab is `Sticker Slab`. The older spellings `Autograph`, `Team` and `Event` are still
  accepted as aliases. Matching is case-insensitive.
</ParamField>
...
curl -sS \
  -H "Authorization: Bearer $CS2C_API_KEY" \
  "https://api.cs2c.app/v1/items/metadata"
```
...
```json
{
  ...
  "filters": {
	...
	"item_subtype": ["Weapon Case", "Player Autograph", "Rifles"],
...
curl -sS \
  -H "Authorization: Bearer $CS2C_API_KEY" \
  "https://api.cs2c.app/v1/images?format=webp"
```
...
curl -sS \
  -H "Authorization: Bearer $CS2C_API_KEY" \
  "https://api.cs2c.app/v1/providers?provider=buff163"
```
...
<Note>
The endpoint returns the exchange rate for **234 different currencies**, including cryptocurrencies. The full list of currencies is available on browser [here](/docs/fx-currencies.json).
</Note>
